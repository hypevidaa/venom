...
__exportStar(require("./ConnectionController/proxy"), exports);
...
class EverscaleStandaloneClient extends utils_1.SafeEventEmitter {
    ...
    constructor(ctx) {
        ...
        this.setPollingInterval = (interval) => {
            ...
            (_a = this._context.subscriptionController) === null || _a === void 0 ? void 0 : _a.setPollingInterval(interval);
        };
        this._context = ctx;
    }
    request(req) {
...
const computeStorageFee = async (ctx, req) => {
    requireParams(req);
    requireConnection(req, ctx);
...
const executeLocal = async (ctx, req) => {
    ...
    if (messageHeader.type === 'external') {
        if (payload == null || typeof payload === 'string') {
            ...
            const unsignedMessage = nekoton.createExternalMessage(clock, repackedAddress, payload.abi, payload.method, stateInit, payload.params, messageHeader.publicKey, timeout);
            try {
...
function requireContractStateBoc(req, object, key) {
    requireObject(req, object, key);
    const property = object[key];
